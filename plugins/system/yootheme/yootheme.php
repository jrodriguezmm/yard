...
class plgSystemYOOtheme extends CMSPlugin
{
    ...
    {
        if (isset($this->app['events'])) {
            $this->app['events']->trigger('content.prepare', array($this->cms->getDocument(), $this->cms->input, $context, &$article, &$params, $page));
        }
    }
    ...
    protected function loadTemplate($templ)
    {
        ...
        $bootstrap = "{$this->root}/templates/{$templ->template}/bootstrap.php";
